...
public class c5_CreatingSequence {
    ...
    public void trouble_maker() {
        Mono<String> trouble = Mono.fromRunnable( ()-> {throw new IllegalStateException();}); //todo: change this line
    ...
    public void generate_programmatically() {
        ...
        Flux<Integer> generateFlux = Flux.generate(
            ...
            () -> Integer.valueOf(1),
            (state, sink) -> {
                sink.next(Integer.valueOf(state));
        ...
        Flux<Integer> createFlux = Flux.create(sink -> {
            //todo: fix following code so it emits values from 0 to 5 and then completes
            IntStream.range(1,6).forEach( i->sink.next(i) );
            sink.complete();
        });
